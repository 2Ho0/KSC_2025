...
class TrajectoryDataset(Dataset):
    ...
    def from_dreamer_batch(cls, dt_batch, max_len, device="cpu"):
        """Create TrajectoryDataset from DreamerV3 batch data"""
        dataset = cls.__new__(cls)
        
        # Set basic attributes
        dataset.max_len = max_len
        dataset.device = device
        dataset.prob_go_from_end = 0
        ...
        B, T = states.shape[:2]
        dataset.states = [states[b] for b in range(B)]
        dataset.actions = [actions[b] for b in range(B)]
        dataset.rewards = [rewards[b] for b in range(B)]
        dataset.dones = [dones[b] for b in range(B)]
        
        # Calculate returns and timesteps
        dataset.returns = [r.sum() for r in dataset.rewards]
        dataset.timesteps = [torch.arange(len(r)) for r in dataset.rewards]
        dataset.traj_lens = np.array([len(r) for r in dataset.rewards])
        
        # Set metadata
        dataset.num_timesteps = sum(dataset.traj_lens)
        dataset.num_trajectories = len(dataset.states)
        dataset.state_dim = list(dataset.states[0][0].shape)
        dataset.act_dim = list(dataset.actions[0][0].shape) if dataset.actions[0][0].ndim > 0 else [1]
        dataset.max_ep_len = max([len(i) for i in dataset.states])
        
        # Set task IDs (assuming single task for now)
        dataset.task_ids = [0] * dataset.num_trajectories
        
        # Set indices for sampling
        dataset.indices = np.arange(dataset.num_trajectories)
        dataset.sampling_probabilities = np.ones(dataset.num_trajectories) / dataset.num_trajectories
        
        # Set normalization parameters
        dataset.state_mean = 0
        dataset.state_std = 1
