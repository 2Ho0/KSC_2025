...
class Agent(embodied.jax.Agent):
  ...
  def __init__(self, obs_space, act_space, config):
    ...
    exclude = ('is_first', 'is_last', 'is_terminal', 'reward','rtg')
  ...
  def policy(self, carry, obs, mode='train'):
    ...
    enc_obs = {k: v for k, v in obs.items() if k != 'rtg'}
  ...
  def train(self, carry, data):
    ...
    metrics, (carry, entries, outs, mets) = self.opt(
        self.loss_imagine, carry, obs, prevact, training=True, has_aux=True)
  ...
  def train_real(self, carry, data):
    carry, obs, prevact, stepid = self._apply_replay_context(carry, data)
    metrics, (carry, entries, outs, mets) = self.opt(
        self.loss_real, carry, obs, prevact, training=True, has_aux=True)
  ...
  def loss_imagine(self, carry, obs, prevact, training): # 원본 그대로
  ...
  def loss_real(self, carry, obs, prevact, training):
    enc_carry, dyn_carry, dec_carry = carry
    reset = obs['is_first']
    B, T = reset.shape
    losses = {}
    ...
    # World model (동일하게 실행하여 repfeat 얻기)
    enc_carry, enc_entries, tokens = self.enc(
        enc_carry, obs, reset, training)
    dyn_carry, dyn_entries, los, repfeat, mets = self.dyn.loss(
        dyn_carry, tokens, prevact, reset, training)
    ...
    for key, recon in recons.items():
      ...
      losses[key] = recon.loss(sg(target))
    ##############################################################################    
    # Imagination (실제 데이터 사용 로직)
    # K = self.config.batch_length - self.config.replay_context # K 값을 정적(static)으로 설정
    # H = self.config.imag_length

    # real_feat_slice = jax.tree.map(lambda x: x[:, -(H + 1):], repfeat)
    # real_act_slice = jax.tree.map(lambda x: x[:, -(H + 1):], prevact)

    # def repeat_and_reshape(real_data):
    #     with_k_dim = jax.tree.map(lambda x: x[:, None, ...], real_data)
    #     repeated = jax.tree.map(lambda x: jnp.repeat(x, K, axis=1), with_k_dim)
    #     reshaped = jax.tree.map(lambda x: x.reshape((B * K, *x.shape[2:])), repeated)
    #     return reshaped

    # imgfeat = repeat_and_reshape(real_feat_slice) 
    # imgact = repeat_and_reshape(real_act_slice)
    ...
    if self.config.repval_loss:
      ...
      boot = obs['rtg']
      feat, last, term, rew, boot = jax.tree.map(
          lambda x: x[:, -T:], (feat, last, term, rew, boot))
      inp = self.feat2tensor(feat)
      los, reploss_out, mets = repl_loss(
          last, term, rew, boot, self.val(inp, 2), self.slowval(inp, 2),
          self.valnorm, update=training, **self.config.repl_loss)
      losses.update(los)
      metrics.update(prefix(mets, 'reploss'))

    # imagination을 안하면서 Policy loss를 계산하기 위해 추가(Behavior Cloning)
    feat_for_actors = self.feat2tensor(repfeat) # 현재 상태 정보를 텐서로 변환
    policy = self.pol(sg(feat_for_actors), 2) # 정책 네트워크를 통해 행동 분포 계산
    log_probs = []
    for key, dist in policy.items():
        # 딕셔너리에서 분포(distribution) 객체를 꺼냅니다.
        # 데이터에 기록된 해당 행동(prevact[key])의 로그 확률을 계산합니다.
        log_prob = dist.logp(sg(prevact[key]))
        log_probs.append(log_prob)

    # 모든 행동 요소의 로그 확률을 합산하여 최종 손실을 계산합니다.
    policy_loss = -sum(log_probs) # 행동 확률의 음의 로그 계산
    losses['policy'] = policy_loss # 정책 손실을 losses에 추가
  ...
  def report(self, carry, data):
    ...
    # Train metrics
    _, (new_carry, entries, outs, mets) = self.loss_imagine(
    ...
    if self.config.report_gradnorms:
      for key in self.scales:
        try:
          lossfn = lambda data, carry: self.loss_imagine(
    ...
    policyfn = lambda feat: sample(self.pol(self.feat2tensor(feat), 1))
    _, imgfeat, _ = self.dyn.imagine(
        dyn_carry, policyfn, length=T - T // 2, training=False)
    ...
    #   video = jnp.concatenate([true, pred, error], 3)
...
def repl_loss(
  ...
  ret_normed_bt = jnp.reshape(ret_normed, ret_normed.shape[:2])
  # ret_padded = jnp.concatenate([ret_normed, 0 * ret_normed[:, -1:]], 1)
  losses['repval'] = weight[:, :-1] * (
      value.loss(sg(ret_normed_bt)) +
      slowreg * value.loss(sg(slowvalue.pred())))[:, :-1]
