...
class RSSM(nj.Module):
  ...
  def loss(self, carry, tokens, acts, reset, training):
    ...
    metrics['dyn_kl_raw'] = dyn_raw.mean()      # unclamped
    metrics['rep_kl_raw'] = rep_raw.mean()      # unclamped
    metrics['dyn_kl'] = dyn.mean()              # clamped (loss에 실제 쓰이는 값)
    metrics['rep_kl'] = rep.mean()
  ...
  def __call__(self, carry, feat, reset, training, single=False):
    ...
    if self.imgkeys:
      ...
      if self.outer:
        ...
        x = self.sub('imgout', nn.Conv2D, self.imgdep, K, **kw)(x)
      # else:
      #   # 목표 shape: (batch, length, height, width, channels)
      #   # x.shape[:-3]는 (batch*length) 부분을 의미
      #   target_shape = (*x.shape[:-3], *self.imgres, x.shape[-1])
      #   # jax.image.resize를 사용하여 목표 크기로 정확하게 확대
