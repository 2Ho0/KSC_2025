...
class Replay:
  ...
  def _insert(self, chunkid, index):
    ...
    if isinstance(self.sampler, (selectors.RTGBased,)):
        # RTGBased 샘플러의 경우, trajectory의 RTG 시퀀스를 가져옵니다.
        rtgs = self._getseq(chunkid, index, ['rtg'])['rtg']
        # 시퀀스의 첫 번째 RTG 값을 우선순위 값으로 사용합니다.
        value = rtgs[0]
        self.sampler[itemid] = value
    else:
        # 그 외 다른 모든 샘플러(Uniform, Prioritized 등)는 기존 방식대로 stepid를 전달합니다.
