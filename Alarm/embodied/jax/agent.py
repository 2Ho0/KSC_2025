...
class Agent(embodied.Agent):
  ...
  def __init__(self, model, obs_space, act_space, config, jaxcfg):
    ...
    L = int(config.batch_length + config.replay_context)
    
    ext_space['task_id'] = elements.Space(np.int32, shape=())
    ext_space['global_step'] = elements.Space(np.int64, shape=())
    # ext_space['rtg'] = elements.Space(np.float32, shape=())
    ...
    self._train = transform.apply(
        ...
        **shared_kwargs)
    self._train_real = transform.apply(
        nj.pure(self.model.train_real), self.train_mesh,
        (dona_sharding, allo_sharding, tm, ts, ts), (tp, ts, ts, tm), ar,
        return_params=True, donate_params=True, first_outnums=(3,),
        **shared_kwargs)
  ...
  def policy(self, carry, obs, mode='train'):
    obs = {**obs, 'rtg': np.array([0.0], dtype=np.float32)} if 'rtg' not in obs else obs
    if not self.jaxcfg.enable_policy:
      raise Exception('Policy not available when enable_policy=False')
  ...
  @elements.timer.section('jaxagent_train_real')
  def train_real(self, carry, data):
    ...
    with self.train_lock:
      with elements.timer.section('jit_train_real'):
        with jax.profiler.StepTraceAnnotation(
            'train_real', step_num=int(self.n_updates)):
          self.params, carry, outs, mets = self._train_real(
              dona, allo, seed, carry, data)
    self.n_updates.increment()
